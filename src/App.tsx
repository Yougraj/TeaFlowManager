...
export default function App() {
  // 1. Core States
  const [activeTab, setActiveTab] = useState<'dashboard' | 'workers' | 'yield' | 'sales' | 'sync'>('dashboard');
  const [workers, setWorkers] = useState<Worker[]>([]);
  const [yields, setYields] = useState<DailyYield[]>([]);
  const [sales, setSales] = useState<Sale[]>([]);
  
  const [syncConfig, setSyncConfig] = useState<SyncConfig>({
    clientId: '383182885994-39ve8q899s67c006n7vukpld6e2aiq.apps.googleusercontent.com', // standard test client
    spreadsheetId: '',
    spreadsheetUrl: '',
    accessToken: null,
    tokenExpiresAt: null,
  ...
  const handleUpdateYieldStatus = (id: string, status: 'Paid' | 'Pending') => {
    saveYields(yields.map(y => y.id === id ? { ...y, paymentStatus: status } : y));
  };
  ...
  const handleImportAllData = (backup: { workers: Worker[]; yields: DailyYield[]; sales: Sale[] }) => {
  ...
  const handleUpdateSyncConfig = (updates: Partial<SyncConfig>) => {
    ...
    localStorage.setItem('estate_sync_config', JSON.stringify(savedRepresentation));
  };
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans text-slate-800" id="main-app-container">
      ...
      <aside className="w-64 bg-leaf-900 text-white flex-col hidden lg:flex shrink-0 border-r border-leaf-800">
        ...
        <nav className="flex-1 py-6 space-y-1">
          ...
          >
            <FileSpreadsheet className="w-4 h-4 opacity-85 text-emerald-450" /> Sheet Sync
          </button>
        </nav>
        ...
        <div className="fixed inset-0 z-50 flex lg:hidden" id="mobile-navigation-drawer">
          ...
          <aside className="relative w-64 max-w-[280px] bg-leaf-900 text-white h-full flex flex-col z-50 animate-in slide-in-from-left duration-200">
            ...
            <nav className="flex-1 py-6 space-y-1">
              ...
              >
                <FileSpreadsheet className="w-4 h-4 text-emerald-450" /> Sheet Sync
              </button>
            </nav>
        ...
        <header className="min-h-16 bg-white border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 px-4 sm:px-8 shrink-0 shadow-2xs gap-3">
          ...
          <div className="flex items-center flex-wrap gap-2 text-xs font-sans w-full sm:w-auto justify-end">
            ...
            <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-slate-100 text-slate-700 rounded-lg text-[10px] font-semibold border border-slate-200 shadow-3xs">
              <div className={`w-1.5 h-1.5 rounded-full ${syncConfig.isLinked ? 'bg-emerald-400 animate-pulse' : 'bg-gray-400'}`}></div>
              <span className="hidden md:inline">{syncConfig.isLinked ? 'Sheet Connected' : 'Local Storage'}</span>
            </div>
          </div>
        </header>
