...
export default function DashboardView({ workers, yields, sales, onNavigate }: DashboardProps) {
  ...
  const totalYieldKg = yields.reduce((acc, y) => acc + y.leavesPlucked, 0).toFixed(1);
  ...
  const avgSalesPricePerKg = totalSalesKg > 0 
    ? (totalSalesRevenue / totalSalesKg).toFixed(2)
    : "0.00";
  ...
  const yieldByDateMap = yields.reduce((acc, y) => {
  ...
  const financesByDateMap = {} as Record<string, { revenue: number; wages: number }>;
  
  sales.forEach(s => {
    if (!financesByDateMap[s.date]) financesByDateMap[s.date] = { revenue: 0, wages: 0 };
    financesByDateMap[s.date].revenue += s.totalAmount;
  });

  yields.forEach(y => {
    if (!financesByDateMap[y.date]) financesByDateMap[y.date] = { revenue: 0, wages: 0 };
  ...
  const pluckerMap = yields.reduce((acc, y) => {
    ...
    <div className="space-y-8" id="dashboard-container">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-xs flex flex-col justify-between hover:border-leaf-200 transition">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-500 font-sans uppercase tracking-wider">Leaves Harvested</span>
            <div className="p-2.5 rounded-lg bg-leaf-50 text-leaf-600">
              <Leaf className="w-5 h-5" />
            </div>
          </div>
          <div className="mt-4">
            <p className="text-2xl font-mono font-semibold text-leaf-900" id="yield-weight-total">
              {totalYieldKg} <span className="text-sm font-sans font-normal text-gray-400">kgs</span>
            </p>
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-xs flex flex-col justify-between hover:border-leaf-200 transition">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-500 font-sans uppercase tracking-wider">Tea Traded Revenue</span>
            <div className="p-2.5 rounded-lg bg-amber-50 text-harvest-800">
              <Coins className="w-5 h-5" />
            </div>
          </div>
          <div className="mt-4">
            <p className="text-2xl font-mono font-semibold text-gray-900" id="sales-income-total">
              ₹{totalSalesRevenue.toLocaleString('en-IN', { minimumFractionDigits: 0 })}
            </p>
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-xs flex flex-col justify-between hover:border-leaf-200 transition">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-500 font-sans uppercase tracking-wider">Gross Wages & Advances</span>
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-xs flex flex-col justify-between hover:border-leaf-200 transition">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-500 font-sans uppercase tracking-wider">Net Operating Surplus</span>
            <div className="p-2.5 rounded-lg bg-[#e8f5e9] text-emerald-800">
              <TrendingUp className="w-5 h-5" />
            </div>
          </div>
          <div className="mt-4">
            <p className={`text-2xl font-mono font-semibold ${operatingProfit >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>
              {operatingProfit >= 0 ? '+' : ''}₹{operatingProfit.toLocaleString('en-IN', { minimumFractionDigits: 0 })}
            </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-amber-50/50 p-6 rounded-xl border border-harvest-100">
        ...
        <div className="flex gap-4">
          ...
          <div>
            <h4 className="font-display font-medium text-harvest-800">Outstanding Worker Wages</h4>
            <p className="text-2xl font-mono font-semibold text-amber-950 mt-1" id="outstanding-wages-payout">
              ₹{outstandingWagesPayable.toFixed(1)}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              Total base wages computed minus cash advances, waiting to be cleared.
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-4 border-t md:border-t-0 md:border-l border-harvest-100 pt-4 md:pt-0 md:pl-6">
          ...
          <div>
            <h4 className="font-display font-medium text-orange-900">Advance Money Logged Today</h4>
            <p className="text-2xl font-mono font-semibold text-orange-950 mt-1">
              ₹{totalAdvances.toFixed(0)}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              Advances given to workers for medical or daily necessities, deducted directly.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col justify-center border-t md:border-t-0 md:border-l border-harvest-100 pt-4 md:pt-0 md:pl-6">
          <p className="text-xs text-gray-500 font-sans">
            Need to pay workers or cash out daily plucking yields? Update payment status in the Harvest Ledger.
          </p>
          <button
            onClick={() => onNavigate('yield')}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-2xs">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-lg font-display font-medium text-gray-900">Leaf Harvest Volume (kg)</h3>
              <p className="text-xs text-gray-400">Trend of leaf plucking weight across the previous 7 active logging dates</p>
            </div>
            <span className="text-2xs font-mono text-leaf-700 bg-leaf-50 px-2 py-1 rounded inline-block">Active Seasons</span>
          </div>
          
          <div className="h-64 w-full">
            {yieldChartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={yieldChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                  <XAxis dataKey="date" stroke="#94a3b8" tickLine={false} style={{ fontSize: '11px', fontFamily: 'monospace' }} />
                  <YAxis stroke="#94a3b8" tickLine={false} style={{ fontSize: '11px', fontFamily: 'monospace' }} />
                  <Tooltip contentStyle={{ fontFamily: 'sans-serif', fontSize: '12px', borderRadius: '8px' }} />
                  <Line 
                    type="monotone" 
              ...
              <div className="h-full flex items-center justify-center text-gray-400 text-sm font-sans italic">
                No harvesting dates logged. Record today's harvest to populate charts.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-2xs">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-lg font-display font-medium text-gray-900">Tea Revenue vs. Labor Outlay</h3>
              <p className="text-xs text-gray-400">Comparison of sales revenue received (₹) vs worker compensations (₹)</p>
            </div>
            <span className="text-2xs font-mono text-harvest-800 bg-harvest-50 px-2 py-1 rounded inline-block">Ledger Finance</span>
          </div>
            ...
            {financialChartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={financialChartData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                  ...
                  <Bar dataKey="Tea Sales (₹)" fill="#ca8a04" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="Worker Wages (₹)" fill="#1b4332" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-full flex items-center justify-center text-gray-400 text-sm font-sans italic">
                No transactions completed. Log some leaf sales to compare balance trends.
              </div>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 bg-white p-6 rounded-xl border border-gray-100 shadow-2xs flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-base font-display font-semibold text-gray-900">Harvest Leaderboard</h3>
            </div>
            <p className="text-xs text-gray-400 mb-6 font-sans">
              Most productive workers based on total cumulative kilograms plucked.
            </p>
            
            <div className="space-y-4">
              {topPluckers.length > 0 ? (
                topPluckers.map((p, index) => (
                  <div key={p.name} className="flex items-center justify-between border-b pb-3 border-gray-50 last:border-0 last:pb-0">
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-gray-100 shadow-2xs">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-display font-semibold text-gray-900">Recent Plucking Ledger Updates</h3>
            <span className="text-2xs text-gray-400 font-mono">Date Sorted</span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-gray-500 font-sans">
              ...
              <tbody className="divide-y divide-gray-50">
                {yields.slice(-4).reverse().map((y) => (
                  <tr key={y.id} className="hover:bg-gray-50/40 transition">
                    <td className="px-4 py-3 font-mono font-medium text-gray-900">{y.date}</td>
                    <td className="px-4 py-3 font-medium text-gray-800">{y.workerName}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded text-3xs font-semibold ${
                        y.activity === 'Plucking' ? 'bg-emerald-50 text-emerald-800' :
                        y.activity === 'Pruning' ? 'bg-orange-50 text-orange-800' :
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="font-mono text-gray-900">₹{y.baseWages.toFixed(0)}</div>
                      {y.cashAdvanceTaken && (
                        <div className="text-3xs text-orange-600 font-mono">Adv: -₹{y.cashAdvanceAmount.toFixed(0)}</div>
                      )}
                    </td>
                ...
                {yields.length === 0 && (
                  <tr>
                    <td colSpan={6} className="text-center py-6 text-gray-400 font-sans italic">
                      No estate logs exist. Set up some workers and record yields to view logs here!
                    </td>
                  </tr>
                )}
              </tbody>
